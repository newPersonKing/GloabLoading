...
public class Gloading {
    ...
    public static class Holder {
        ...
        private ViewGroup mWrapper; /*显示loading的根view*/
        private int curState;
        private SparseArray<View> mStatusViews = new SparseArray<>(4);/*存储四种状态对应的view*/
        ...
        public void showLoadingStatus(int status) {
            ...
            View convertView = mStatusViews.get(status); /*获取对应状态对应的view*/
            if (convertView == null) {
                //secondly try to reuse current status view
                convertView = mCurStatusView;
            }
            Log.i("cccccccccccccc","convertView==="+convertView);
            try {
                //call customer adapter to get UI for specific status. convertView can be reused
                View view = mAdapter.getView(this, convertView, status); /*根据不同的状态返回不同的view*/
                if (view == null) {
                    printLog(mAdapter.getClass().getName() + ".getView returns null");
                    Log.i("cccccccccccccc","getView returns null");
                    /*这里如果不设置 什么都没做 自己添加 移除上一个状态的view*/
                ...
                if (view != mCurStatusView || mWrapper.indexOfChild(view) < 0) {
                    ...
                    Log.i("ccccccccc","childCount==="+mWrapper.getChildCount());
                    ViewGroup.LayoutParams lp = view.getLayoutParams();
                    if (lp != null) {
